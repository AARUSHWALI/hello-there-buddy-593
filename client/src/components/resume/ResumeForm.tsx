...
import { FileText, Trash, Plus, Save, Code, CheckCircle, Mail } from "lucide-react";
...
export default function ResumeForm({ resumeData, setResumeData, parsedFile, jsonData, setJsonData }: ResumeFormProps) {
  ...
  const { isSending, sendTestInvite } = usePersonalityTest();
  ...
  const handleSendTestInvite = async () => {
    if (!resumeData?.personalInfo?.email || !resumeData?.personalInfo?.name) {
      toast({
        title: "Missing Information",
        description: "Candidate email and name are required to send the test invitation.",
        variant: "destructive",
      });
      return;
    }
        ...
        <div className="flex justify-between items-center">
          ...
          <div className="flex gap-4">
            ...
            <Button 
              ...
              disabled={isSending || !resumeData?.personalInfo?.email || !resumeData?.personalInfo?.name}
            >
              {isSending ? (
                <>
                  <svg className="animate-spin -ml-1 mr-2 h-4 w-4 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                ...
                <>
                  <Mail className="w-4 h-4 mr-2" /> Send Personality Test
                </>
              )}
            </Button>
          </div>
