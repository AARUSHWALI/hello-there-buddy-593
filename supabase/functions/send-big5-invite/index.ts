...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    // Use production URL directly for consistency
    const PRODUCTION_URL = 'https://people-ai-new.vercel.app';
    const testUrl = `${PRODUCTION_URL}/personality-test?token=${token}&email=${encodeURIComponent(candidateEmail)}&name=${encodeURIComponent(candidateName)}${resumeId ? `&resumeId=${resumeId}` : ''}`;
        ...
        <html>
          ...
          <body>
            <div class="container">
              ...
              <div style="word-break: break-all; color: #8B5CF6; font-size: 14px; background-color: #f3f4f6; padding: 10px; border-radius: 4px; margin: 10px 0;">${testUrl}</div>
