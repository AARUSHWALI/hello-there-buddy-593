...
//     // Create the test URL with the token - get origin from request or use production URL
//     const origin = req.headers.get('origin') || req.headers.get('referer')?.split('/').slice(0, 3).join('/') || 'https://people-ai-new.vercel.app';
//     const testUrl = `${origin}/personality-test?token=${token}&email=${encodeURIComponent(candidateEmail)}&name=${encodeURIComponent(candidateName)}${resumeId ? `&resumeId=${resumeId}` : ''}`;
...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    // Create the test URL with the token.
    // This logic dynamically determines the host (origin) or falls back to the Vercel production URL.
    const origin = req.headers.get('origin') || req.headers.get('referer')?.split('/').slice(0, 3).join('/') || 'https://people-ai-new.vercel.app';
    const testUrl = `${origin}/personality-test?token=${token}&email=${encodeURIComponent(candidateEmail)}&name=${encodeURIComponent(candidateName)}${resumeId ? `&resumeId=${resumeId}` : ''}`;
