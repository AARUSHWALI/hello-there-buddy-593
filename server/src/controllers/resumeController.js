...
      if (Object.keys(rest).length > 0) {
        rawResumeData = rest;
        console.log('Using data from request body:', JSON.stringify(rawResumeData, null, 2));
      }
    }
    
    // Handle file upload if present
...
    // --- START NEW DB INSERT PAYLOAD ---
    // We create a specific payload to map key fields and safely insert the complex object.
    const dbPayload = {
      // Indexable/Queryable fields (MUST match table schema exactly)
      name: rawResumeData.personalInfo?.name || null,
      email: rawResumeData.personalInfo?.email || null,
      phone: rawResumeData.personalInfo?.phone || null, // Varchar/Text in DB
      address: rawResumeData.personalInfo?.address || null, // Text in DB
      summary: rawResumeData.personalInfo?.summary || null, // Text in DB
      ug_institute: rawResumeData.ug_institute || null, // Varchar/Text in DB
      pg_institute: rawResumeData.pg_institute || null, // Varchar/Text in DB
      phd_institute: String(rawResumeData.phd_institute || 0), // Explicitly cast to string for character varying column
      longevity_years: rawResumeData.longevity_years || 0, // Integer in DB
      number_of_jobs: rawResumeData.number_of_jobs || 0, // Integer in DB
      average_experience: rawResumeData.average_experience || 0, // Numeric/Float in DB
      skills_count: rawResumeData.skills_count || 0, // Integer in DB
      achievements_count: rawResumeData.achievements_count || 0, // Integer in DB
      trainings_count: rawResumeData.trainings_count || 0, // Integer in DB
      workshops_count: rawResumeData.workshops_count || 0, // Integer in DB
      projects_count: rawResumeData.projects_count || 0, // Integer in DB
      research_papers_count: rawResumeData.research_papers_count || 0, // Integer in DB
      patents_count: rawResumeData.patents_count || 0, // Integer in DB
      books_count: rawResumeData.books_count || 0, // Integer in DB
      is_jk: (rawResumeData.is_jk || 0) === 1 ? 1 : 0, // CONVERTED TO INTEGER (1 or 0)
      best_fit_for: rawResumeData.best_fit_for || null, // Varchar/Text in DB

      // JSONB fields (Complex array/object data)
...
    console.log('Inserting database payload:', JSON.stringify(dbPayload, null, 2));
...
        hint: dbError.hint,
        code: dbError.code,
        payload: dbPayload // Log the payload that failed
      });
      // We throw an error here to prevent responding with a false success status
      throw new Error(`Database operation failed: ${dbError.message}`);
    }
...
      ...(process.env.NODE_ENV === 'development' && { 
        stack: error.stack,
        // The detailed database error object might not always be accessible here, 
        // but it is logged right before the re-throw.
      })
    });
  } finally {
    console.log('=== End of createResume ===');
...
// Get resume file by ID
export const getResumeFile = async (req, res) => { // <-- EXPORT ADDED HERE
  try {
    const { id } = req.params;
    console.log(`Getting file for resume ID: ${id}`);
    
